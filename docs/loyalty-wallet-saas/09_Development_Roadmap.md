...
منع تكرار إضافة نقاط/أختام لنفس العميل خلال فترة زمنية قصيرة (مثال: Stampli
تمنع تكرار الختم لنفس البطاقة خلال دقائق). يجب تطبيقها على مستوى الـ
transaction قبل الإدراج (تحقق من آخر عملية للعميل + رسالة رفض للموظف عند
محاولة التكرار السريع).
...
بعد ربط Wallet Provider، استخدام قناة الإشعارات الخاصة به لإرسال رسائل تظهر
على شاشة القفل (عروض، تذكير بمكافأة جاهزة للاستبدال، وصول لعتبة نقاط). تعتمد
على نفس Wallet Service (`updateCard()` يُطلق push تلقائيًا عند أي تحديث على
البطاقة عبر أغلب المزوّدين مثل PassKit/Passcreator).
