...
export function QrScanner({
  ...
  // Guards the detect loop: without it, a QR sitting in frame fires the
  ...
  const start = useCallback(async () => {
    ...
    if (!navigator.mediaDevices?.getUserMedia) {
      setError("المتصفح لا يدعم الكاميرا. جرّب Chrome أو Safari حديث.");
      setPhase("idle");
      return;
    }
    ...
    try {
      ...
      setError(
        name === "NotAllowedError"
          ? "لم يُسمح باستخدام الكاميرا. فعّل الإذن من إعدادات المتصفح."
          : name === "NotFoundError"
  ...
  return (
    <div className="mt-6">
      ...
      {/* The video element stays mounted across phases: remounting it forces
          the browser to re-acquire the camera and re-prompt on some devices. */}
      <div className={phase === "scanning" ? "block" : "hidden"}>
        <div className="relative overflow-hidden rounded-xl bg-black">
          ...
          <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
            <div className="h-48 w-48 rounded-2xl border-4 border-accent-500/80" />
          </div>
        </div>
        <p className="mt-3 text-center text-sm text-primary-500">
          وجّه الكاميرا نحو الرمز في بطاقة العميل
        </p>
        <button
          type="button"
          onClick={reset}
          className="min-h-touch mt-3 w-full rounded-xl border border-primary-300 px-5 py-3 text-sm font-medium"
        >
          إلغاء
        </button>
      </div>
          ...
          {canAddPoints ? (
            <>
              ...
              <input
                ...
                className="min-h-touch mt-1 w-full rounded-lg border border-primary-300 px-3 py-2 text-base dark:border-primary-700 dark:bg-primary-900"
              />
              <button
                type="button"
                onClick={confirm}
                disabled={pending}
                className="min-h-touch mt-3 w-full rounded-xl bg-accent-500 px-5 py-4 text-base font-semibold text-white disabled:opacity-50"
              >
      ...
      {phase === "done" && result && card && (
        <div className="rounded-xl border-r-4 border-success-500 bg-success-50 p-4 text-center">
          ...
          <button
            type="button"
            onClick={start}
            className="min-h-touch mt-4 w-full rounded-xl bg-primary-800 px-5 py-4 text-base font-semibold text-white"
          >
            امسح بطاقة أخرى
          </button>
        </div>
